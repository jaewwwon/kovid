...
import PostCommentItem from "../../../components/Content/PostCommentItem";
import { useSelector, useDispatch } from "react-redux";
import { LOAD_USER_COMMENT_REQUEST } from "../../../reducers/post";

const UserPosts = () => {
  const { query, pathname } = useRouter();
  const dispatch = useDispatch();
  const profile = useSelector((state) => state.user.profile);
  const { userComments, loadPostLoading } = useSelector((state) => state.post);
  const [page, setPage] = useState(query?.page || 1);

  useEffect(() => {
    if (query?.id)
      dispatch({
        type: LOAD_USER_COMMENT_REQUEST,
        data: {
          page: query?.page || 1,
        ...
        <>
          ...
          {userComments.count > 0 ? (
            userComments.comments?.map((comment) => (
              <PostCommentItem key={comment.comment_id} comment={comment} />
            ))
          ) : (
            <p className="none_content">작성한 게시글이 없습니다.</p>
          )}
          {/* 페이지네이션 */}
          {userComments.count >= 10 && (
            <PaginationWrap
              current={page}
              onChange={onChangePage}
              total={userComments.count}
              pageSize={10}
              showTitle={false}
              showLessItems
            />
